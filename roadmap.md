...
**Correction: the `scarcity_z` control comparison — previously reported as a clean 2-and-2 split — does not hold up.** This roadmap and both `06c_model_wr.ipynb` and `06d_model_te.ipynb` previously stated that QB and RB show `scarcity_z`'s own extremes (top/bottom deciles) fitting *better* than the middle 80%, with only WR and TE breaking that pattern (top decile — the actual stars — being the *hardest* segment). **That QB/RB claim was never backed by real, decile-separated notebook code** — it came from an *averaged* top+bottom-decile number computed outside any notebook, which happens to land close to (or better than) the overall MAE and masks what's actually happening in each decile individually. Checked properly (by decile, not averaged) for RB in `06b_model_rb.ipynb`:
- RB's own **top decile** (its actual star backs): MAE 68.53 — **1.47x worse than overall**, and worse than even RB's own delta extreme-tail bucket (64.46).
- RB's own **bottom decile** (replacement level): MAE 26.70 — 0.57x of overall, easy, consistent with every position.
- Combined average of both deciles: 47.62, close to RB's overall held-out MAE (46.65) — this is the number that had previously been (mis)read as "extremes fit better."

**This is now 3 of 3 positions actually checked this decile-separated way (WR, TE, RB) showing the same pattern**: the model's own best players are the hardest segment to predict, not the easiest, and averaging the two tails together hides it. **QB's version of this claim has still never been checked with real, committed decile-separated code in `06a_model_qb.ipynb`** and should be treated as open, not settled, until it is — the working theory of a "receiving-volume vs. run/pocket-presence" position-family split (WR/TE vs. QB/RB) does not survive RB's own real check and should not be repeated as if it does.
